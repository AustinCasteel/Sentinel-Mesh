...
def _build_openai(settings: Settings) -> BaseChatModel:
    ...
    return ChatOpenAI(
        ...
        api_key=settings.openai_api_key or "not-needed",  # type: ignore[arg-type]
...
def _build_lemonade(settings: Settings) -> BaseChatModel:
    ...
    return ChatOpenAI(
        ...
        api_key="lemonade",  # type: ignore[arg-type]
...
def _build_bedrock(settings: Settings) -> BaseChatModel:
    ...
    """
    try:
        from langchain_aws import ChatBedrockConverse  # type: ignore[import-untyped]
...
_BUILDERS: dict[LLMProvider, callable] = {  # type: ignore[type-arg]
...
def get_llm(
    ...
    logger.info("LLM initialised: provider=%s  model=%s", provider.value, llm.model_name)
    return llm
